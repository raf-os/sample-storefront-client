...
import { serverCachedRequest, serverRequest } from "../serverRequest";
...
export async function GetProductComments(productId: string, lastId?: string, lastDate?: string) {
  const urlParams = new URLSearchParams();
  if (lastId !== undefined && lastDate !== undefined) {
    urlParams.append('lastId', lastId);
    urlParams.append('lastDate', String(lastDate));
  }
  const data = await serverRequest("get", "/api/Comment/{Id}", { path: { Id: productId } }, { useAuth: true });
  return data;
}
